...
export function assertLocalActionCompilerContracts() {
  ...
  const mapPinUpdate = compileLocalAction('Update map pin lead ID: cmr23g4zd000otq2c3fuy5wes. No answer. Note: Husband gets back at 4, follow up later.')
  assert(mapPinUpdate?.id === 'update-map-pin', `Map pin update should compile as update-map-pin, got ${mapPinUpdate?.id}`)
  assert(mapPinUpdate.status === 'ready', 'Map pin update with lead ID and status/note should be ready')
  assert(mapPinUpdate.toolCall?.name === 'update_canvassing_lead', `Map pin update should call update_canvassing_lead, got ${mapPinUpdate.toolCall?.name}`)
  assert(mapPinUpdate.toolCall.args.leadId === 'cmr23g4zd000otq2c3fuy5wes', `Map pin update leadId should parse, got ${String(mapPinUpdate.toolCall.args.leadId)}`)
  assert(mapPinUpdate.toolCall.args.status === 'no_answer', `Map pin update status should parse, got ${String(mapPinUpdate.toolCall.args.status)}`)
  assert(String(mapPinUpdate.toolCall.args.notes).includes('Husband gets back at 4'), `Map pin note should parse, got ${String(mapPinUpdate.toolCall.args.notes)}`)
  assert(canRunLocalActionBeforeAi(mapPinUpdate), 'Map pin update should be eligible for pre-AI execution')

  const mapPinConversation = compileLocalAction('Update map pin lead ID: cmr23g4zd000otq2c3fuy5wes. Conversation. Note: Homeowner said husband gets home at 4.')
  assert(mapPinConversation?.id === 'update-map-pin', `Map pin conversation should compile as update-map-pin, got ${mapPinConversation?.id}`)
  assert(mapPinConversation.status === 'ready', 'Map pin conversation with lead ID should be ready')
  assert(mapPinConversation.toolCall?.args.status === 'conversation', `Map pin conversation status should parse, got ${String(mapPinConversation.toolCall?.args.status)}`)
  assert(String(mapPinConversation.toolCall?.args.notes).includes('husband gets home at 4'), `Map pin conversation note should parse, got ${String(mapPinConversation.toolCall?.args.notes)}`)

  const mapPinNote = compileLocalAction('Save note to field map pin lead ID: cmr23g4zd000otq2c3fuy5wes: Already has another roofer waiting on insurance paperwork.')
  assert(mapPinNote?.id === 'update-map-pin', `Map pin note should compile as update-map-pin, got ${mapPinNote?.id}`)
  assert(mapPinNote.status === 'ready', 'Map pin note with lead ID should be ready')
  assert(mapPinNote.toolCall?.name === 'update_canvassing_lead', `Map pin note should call update_canvassing_lead, got ${mapPinNote.toolCall?.name}`)
  assert(mapPinNote.toolCall.args.leadId === 'cmr23g4zd000otq2c3fuy5wes', 'Map pin note should pass leadId')
  assert(!('status' in mapPinNote.toolCall.args), 'Map pin note should not invent a status')
  assert(String(mapPinNote.toolCall.args.notes).includes('Already has another roofer'), `Map pin note should parse, got ${String(mapPinNote.toolCall.args.notes)}`)
  assert(canRunLocalActionBeforeAi(mapPinNote), 'Map pin note should be eligible for pre-AI execution')
