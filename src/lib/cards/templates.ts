...
export const JOBROLO_CARD_TEMPLATES: JobroloCardTemplate[] = [
  ...
  },
  {
    ...
    family: 'integration',
    cardTypes: ['company_phone_numbers', 'company_phone_number_search', 'company_phone_number_provision', 'phone_auth_setup'],
    purpose: 'Show company-owned Jobrolo phone numbers, Twilio readiness, SMS/voice routing status, and safe provisioning prompts.',
    glanceLabel: 'Company phone numbers',
    speakableSummary: 'Here are the company communication numbers and setup status.',
    primaryActionLabel: 'Manage number',
    primaryPromptPills: [
      { label: 'Show numbers', promptPattern: 'Show my saved Jobrolo company phone numbers and explain which one is active for SMS.' },
      { label: 'Search area code', promptPattern: 'Search available Jobrolo phone numbers in area code ' },
      { label: 'SMS setup', promptPattern: 'Check whether my company SMS and phone verification setup is ready. Tell me what is missing without showing secrets.' },
      { label: 'Get Jobrolo number', promptPattern: 'Help me get a Jobrolo company phone number. Search available numbers first and ask before provisioning anything that costs money.' },
    ],
    displayRules: [
      'Searching numbers is safe; provisioning/buying a number must be approval-gated.',
      'Show active/inactive, SMS/voice capability, purpose, and A2P status.',
      'Do not claim customer texting is fully ready until Twilio credentials and business messaging setup are configured.',
      'Keep phone number setup separate from customer phone records and lead intake.',
    ],
  },
  {
    id: 'customer-file',
  ...
  },
  {
    ...
    family: 'signature',
    cardTypes: ['signature_request', 'signature_status', 'signing_request'],
    purpose: 'Show signature requests, signer status, signed document artifacts, audit trail, and safe reminder/finalization actions.',
    glanceLabel: 'Signature request',
    speakableSummary: 'Here’s the signature request card with signer, document, status, audit trail, and safe next actions.',
    primaryActionLabel: 'Review signature',
    primaryPromptPills: [
      { label: 'Check status', promptPattern: 'Check the saved status of this signature request and show signer, document, audit events, and next step.' },
      { label: 'Send reminder', promptPattern: 'Draft a signature reminder for this signer and ask before sending anything.' },
      { label: 'Open signed doc', promptPattern: 'Show the signed document or signing artifact if it is saved and accessible.' },
    ],
    displayRules: [
      'Signature cards must show signer, document, status, and audit state before actions.',
      'Do not expose private signing tokens, signer links, or signed files to unauthorized users.',
      'Creating requests, sending reminders, finalizing documents, or sharing signed PDFs requires approval and permission checks.',
      'Signed documents should render as openable file cards when a safe URL is available.',
    ],
  },
  {
    id: 'approval-action',
  ...
  },
  {
    id: 'field-map',
    family: 'field',
    cardTypes: ['field_map_card', 'field_map', 'map_layer', 'geo_event', 'location_event'],
    purpose: 'Show provider-independent field map truth: current position, property pins, leads, door attempts, photos, signatures, and future AR capture points.',
    glanceLabel: 'Field map',
    speakableSummary: 'Here’s the field map card with GPS evidence, saved pins, nearby leads, and property activity layers.',
    primaryActionLabel: 'Open map',
    primaryPromptPills: [
      { label: 'Open full map', promptPattern: 'Open the full field map for this area with saved pins, nearby leads, and GPS evidence.' },
      { label: 'Drop pin', promptPattern: 'Drop a field lead pin at my current location and let me add homeowner, address, status, and notes.' },
      { label: 'Nearby leads', promptPattern: 'Show nearby field leads and door outcomes from saved map records.' },
      { label: 'Photo layer', promptPattern: 'Show photo evidence on this property map, grouped by where each photo was captured.' },
    ],
    displayRules: [
      'A property may have many location events; do not collapse everything into one address pin.',
      'The map card owns the data contract; OpenStreetMap, Google, Mapbox, and AR are only renderers.',
      'Show GPS evidence separately from confirmed customer/project/property truth.',
      'Map actions should insert editable prompts unless they are already safe local actions.',
      'Future AR/glasses views must consume the same field map points and layers.',
    ],
  },
  {
    id: 'field-inspection',
  ...
  },
  {
    ...
    family: 'project',
    cardTypes: ['production_status', 'project_readiness', 'job_ready', 'material_readiness', 'project_closeout', 'closeout_checklist'],
    purpose: 'Show job readiness, closeout readiness, blockers, material status, approvals, schedule, documents, financial truth, and next production actions.',
    glanceLabel: 'Production / closeout',
    speakableSummary: 'Here’s the production and closeout card with readiness, blockers, missing records, and next actions.',
    ...
    primaryPromptPills: [
      ...
      { label: 'Ready to close?', promptPattern: 'Check whether this job is ready to close using saved project status, final report, signatures, invoices, payments, job costs, commission, warranty packet, and unresolved blockers.' },
  ...
  },
  {
    id: 'insurance-claim',
    family: 'claim',
    cardTypes: ['insurance_claim', 'claim_summary', 'supplement_review', 'pa_claim_file', 'public_adjuster_claim', 'appraisal_tracker', 'payment_control'],
    purpose: 'Organize insurance claim and public-adjuster claim-file data, deductible/RCV/ACV math, supplements, adjuster context, appraisal/payment control, mortgage checks, and claim document gaps.',
    glanceLabel: 'Insurance claim',
    speakableSummary: 'Here’s the insurance or PA claim card with claim facts, phase, financials, documents, gaps, collaboration, and next actions.',
    ...
    primaryPromptPills: [
      ...
      { label: 'PA file phase', promptPattern: 'Classify this public adjuster claim file phase and show the bottleneck, missing info, priority, and next task.' },
      { label: 'Appraisal tracker', promptPattern: 'Show appraisal status, carrier appraiser context, meeting date, packet readiness, and next follow-up from saved records.' },
      { label: 'Share safely', promptPattern: 'Prepare a scoped shared chat/update for the homeowner, contractor, PA, carrier adjuster, or trade. Show visibility before sharing.' },
    ],
    displayRules: [
      ...
      'For PA files, show the claim phase, owner lane, missing paperwork/data, active bottleneck, and next safe task.',
      'Contractor/homeowner/PA/carrier/trade collaboration must be explicit through shared chats and visibility rules.',
      'Do not expose internal strategy, pricing, margin, or private notes to external shared chats by default.',
    ],
  },
  {
    id: 'shared-chat',
  ...
  {
    id: 'canvassing-game-plan',
    family: 'field',
    cardTypes: ['canvassing_game_plan', 'street_game_plan', 'street_research'],
    purpose: 'Show a field/canvassing plan with route strategy, target doors, conversation goals, risks, and prompt-driven next actions.',
    glanceLabel: 'Field game plan',
    speakableSummary: 'Here’s the field game plan with recommended route, goals, and next actions.',
    primaryActionLabel: 'Work plan',
    primaryPromptPills: [
      { label: 'Start route', promptPattern: 'Start this field game plan and help me log doors, conversations, no answers, and follow-ups.' },
      { label: 'Open map', promptPattern: 'Open the field map for this game plan and use my current location.' },
      { label: 'Adjust plan', promptPattern: 'Adjust this field plan based on my current location, time available, and best nearby opportunities.' },
    ],
    displayRules: [
      'Keep the plan compact and field-ready.',
      'Do not navigate away from chat unless the user opens the map.',
      'Each next action should insert an editable prompt or open a scoped field surface.',
    ],
  },
  {
    id: 'command-shortcuts',
    family: 'chat',
    cardTypes: ['command_shortcuts'],
    purpose: 'Show editable prompt shortcuts and shortcut sections without treating them as executed work.',
    glanceLabel: 'Command shortcuts',
    speakableSummary: 'Here are the command shortcuts. Tapping one should insert an editable prompt into chat.',
    primaryActionLabel: 'Edit shortcuts',
    primaryPromptPills: [
      { label: 'Edit shortcuts', promptPattern: 'Help me edit my command shortcuts. Show current sections, prompts, and what can be changed.' },
      { label: 'Add shortcut', promptPattern: 'Add a new command shortcut. Ask for section, title, and exact prompt.' },
      { label: 'Suggest shortcuts', promptPattern: 'Suggest useful shortcuts from my recent Jobrolo usage and ask before saving them.' },
    ],
    displayRules: [
      'Shortcuts insert prompts; they do not silently mutate data.',
      'Show prompt text clearly enough to edit before sending.',
      'Shortcut actions that send, delete, approve, or mutate records must still use normal approval/tool rules.',
    ],
  },
  {
    id: 'brain-memory',
    family: 'meta',
    cardTypes: ['brain_memory_saved', 'brain_context', 'brain_reflection', 'agent_lesson_saved'],
    purpose: 'Show Jobrolo memory, reflections, and learned operating notes as supporting context, never as database truth.',
    glanceLabel: 'Jobrolo memory',
    speakableSummary: 'Here’s Jobrolo memory context. Saved database records still remain the source of truth.',
    primaryActionLabel: 'Review memory',
    primaryPromptPills: [
      { label: 'Explain memory', promptPattern: 'Explain this Jobrolo memory and whether it is a saved fact, preference, or reflection.' },
      { label: 'Use carefully', promptPattern: 'Use this memory only as context and verify against saved records before acting.' },
      { label: 'Forget/update', promptPattern: 'Help me update or remove this Jobrolo memory if it is wrong or no longer useful.' },
    ],
    displayRules: [
      'Memory is advisory context; database records are truth.',
      'Show source/category and confidence when available.',
      'Do not use memory alone to route customer/project/file mutations.',
    ],
  },
  {
    id: 'tester-feedback',
    family: 'meta',
    cardTypes: ['tester_feedback'],
    purpose: 'Capture owner/tester bug notes and product feedback for Cody/Codex review with enough context to reproduce.',
    glanceLabel: 'Tester feedback',
    speakableSummary: 'Here’s tester feedback captured for Cody review.',
    primaryActionLabel: 'Build Cody packet',
    primaryPromptPills: [
      { label: 'Make Cody packet', promptPattern: 'Turn this tester feedback into a Cody packet with route, chat context, expected behavior, actual behavior, likely files, and tests.' },
      { label: 'Add screenshot', promptPattern: 'Add the latest screenshot or upload evidence to this Cody feedback item.' },
      { label: 'Archive', promptPattern: 'Archive this Cody feedback item after confirming it is fixed, duplicated, or intentionally deferred.' },
    ],
    displayRules: [
      'Keep feedback concise and evidence-focused.',
      'Prefer exact route, action, and recent chat context over generic summaries.',
      'Do not expose Cody/tester feedback to non-admin users.',
    ],
  },
  {
    id: 'connection-onboarding',
    family: 'permission',
    cardTypes: ['connection_onboarding'],
    purpose: 'Guide invited users into the correct company/workspace/chat with role and permission clarity.',
    glanceLabel: 'Connection onboarding',
    speakableSummary: 'Here’s the connection onboarding card with role, company, invite, and access status.',
    primaryActionLabel: 'Review access',
    primaryPromptPills: [
      { label: 'Review role', promptPattern: 'Review this user’s role and explain what they can and cannot access.' },
      { label: 'Invite status', promptPattern: 'Show the invite status, target company/workspace, and whether approval is still needed.' },
      { label: 'Fix access', promptPattern: 'Help me fix this user access issue without exposing private records or bypassing approval.' },
    ],
    displayRules: [
      'Show company/workspace linkage clearly.',
      'Do not grant access from a card without permission checks.',
      'External users should only see their allowed chats, files, and tasks.',
    ],
  },
  {
    id: 'cody-review',
