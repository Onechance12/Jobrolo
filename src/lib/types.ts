...
export interface MessageAttachment { type: 'image' | 'file' | 'link'; name: string; url: string; thumbnailUrl?: string; mimeType: string; size?: number; documentId?: string; documentStatus?: 'queued' | 'processing' | 'reviewed' | 'failed' | 'needs_ocr' | 'pending_review'; documentType?: string; documentSummary?: string; documentCategory?: string; documentExtractedData?: Record<string, unknown> | null; description?: string; source?: string }
